...
class TestList(unittest.TestCase):
    ...
    def test_02h(self) -> None:
        ht = HashTable(7)
        ht.insert('!"', [5])
        ht.insert("b", [12])
        self.assertEqual(ht.get_index("b"), 1)
    ...
    def test_setitem_getitem_contains(self) -> None:
        ht = HashTable(7)
        ht["cat"] = 5
        self.assertEqual(ht["cat"], 5)
        ht["cat"] = 7
        self.assertEqual(ht["cat"], 7)
        self.assertEqual(ht["dog"], None)
        self.assertEqual(ht.get_all_keys(), ["cat"])
        self.assertEqual(ht.get_num_items(), 1)
        self.assertTrue("cat" in ht)
        self.assertFalse("dog" in ht)
